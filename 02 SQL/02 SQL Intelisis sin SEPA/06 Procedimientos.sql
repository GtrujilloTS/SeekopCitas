...
*/
ALTER PROCEDURE [dbo].[xpCA_HorarioAgenteNombres](@Sucursal int, @Anio VARCHAR(4), @Mes VARCHAR(2), @Dia VARCHAR(2),
                                                  @Interfaz VARCHAR(20))
AS
BEGIN
    ...
    IF @FechaConMargen > (CONVERT(datetime,
                @Anio + '-' + REPLACE(STR(@Mes, 2), ' ', '0') + '-' + REPLACE(STR(@Dia, 2), ' ', '0') + 'T' + '19:00' +
                ':00.000', 126))/*El horario con margen es mayoy a la fecha de consulta*/
        BEGIN
    ...
    ELSE
        BEGIN
            ...
            IF (SUBSTRING(CONVERT(VARCHAR(23), @FechaConMargen, 126), 12, 5)) > ('19:00')
                ...
                END
            ELSE
                IF (SUBSTRING(CONVERT(VARCHAR(23), @FechaConMargen, 126), 12, 5)) < ('07:00')
                    BEGIN
                        SELECT @HoraComienzo = '07:00'
                    END
                ELSE
                    IF @FechaConMargen < (CONVERT(datetime,
                                @Anio + '-' + REPLACE(STR(@Mes, 2), ' ', '0') + '-' + REPLACE(STR(@Dia, 2), ' ', '0') +
                                'T' + '07:00' + ':00.000', 126))--(CONVERT(VARCHAR(10),@FechaConMargen,103))=(CONVERT(VARCHAR(10),GETDATE(),103))
                        BEGIN
                            SELECT @HoraComienzo = '07:00'
                        END
                    ELSE
    ...
    SELECT Agente,
           Jornada,
           FechaEmision,
           ISNULL(HoraRecepcion, HoraRequerida) as HoraRecepcion,
           FechaRequerida,
           HoraRequerida,
           dtRequeridaIni,
           dtRequeridaFin
    FROM vwCA_CCVistaAgentes
    WHERE YEAR(FechaRequerida) = @Anio
      AND MONTH(FechaRequerida) = @Mes
      AND DAY(FechaRequerida) = @Dia
      AND Jornada IS NOT NULL
      AND dbo.fnCA_UtilitiesCheckTime(HoraRecepcion) IS NOT NULL
      AND dbo.fnCA_UtilitiesCheckTime(HoraRequerida) IS NOT NULL
    ...
    SELECT L.Agente,
           ...
           L.FechaEmision,
           L.HoraRequerida,
           CONVERT(datetime, CONVERT(varchar(10), L.FechaEmision, 126) + 'T' + CASE
                                                                                   WHEN ISDATE(L.HoraRequerida) = 1 AND CHARINDEX(':', L.HoraRequerida) > 0
                                                                                       THEN L.HoraRequerida
                                                                                   ELSE '00:00' END + ':00.000', 126),
           DATEADD(MI, 15, CONVERT(datetime, CONVERT(varchar(10), L.FechaEmision, 126) + 'T' + CASE
                                                                                                   WHEN ISDATE(L.HoraRequerida) = 1 AND CHARINDEX(':', L.HoraRequerida) > 0
                                                                                                       THEN L.HoraRequerida
                                                                                                   ELSE '00:00' END +
                                             ':00.000', 126))
    ...
    WHERE YEAR(L.FechaEmision) = @Anio
      AND MONTH(L.FechaEmision) = @Mes
      AND DAY(L.FechaEmision) = @Dia
    ...
    IF EXISTS(SELECT * FROM SYSOBJECTS WHERE ID = OBJECT_ID('dbo.CA_Asesores') and type = 'U')/*Revisamos si existe la tabla para poder sacar la prospoeccion*/
        BEGIN
            ...
            SELECT Agente1,
                   A.Jornada,
                   CA.Fecha,
                   CASE WHEN CA.Horario = '' THEN '00:00' ELSE CA.Horario END,
                   CA.Fecha,
                   CASE WHEN CA.Horario = '' THEN '00:00' ELSE CA.Horario END,
                   CONVERT(datetime, CONVERT(VARCHAR(10), CA.Fecha, 126) + 'T' +
                                     CASE WHEN CA.Horario = '' THEN '00:00' ELSE CA.Horario END + ':00.000', 126),
                   CONVERT(datetime, CONVERT(VARCHAR(10), CA.Fecha, 126) + 'T' +
                                     CASE WHEN CA.Horario = '' THEN '00:00' ELSE CA.Horario END + ':00.000', 126)
            FROM CA_Asesores AS CA
                     INNER JOIN Agente AS A ON CA.Agente1 = A.Agente
            WHERE SUCURSAL = @Sucursal
              AND YEAR(CA.Fecha) = @Anio
              AND MONTH(CA.Fecha) = @Mes
              AND DAY(CA.Fecha) = @Dia
            UNION
            SELECT Agente2,
                   A.Jornada,
                   CA.Fecha,
...
---------------BUSCAMOS LOS ASESORES O TECNICOS CON UNA JORNADA ASIGNADA ------------------------------
    IF @Interfaz = 'SePa'
        BEGIN
            IF EXISTS(SELECT *
                      FROM CA_HorarioAgenteRecepcion AS H
                               INNER JOIN AGENTE A ON H.Agente = A.Agente
                      WHERE A.SucursalEmpresa = @Sucursal)
                BEGIN
                    DECLARE HorarioJornada CURSOR FOR
                        SELECT DISTINCT A.Jornada
                        FROM Agente AS A
                                 INNER JOIN CA_HorarioAgenteRecepcion AS H ON A.Agente = H.Agente + ''
                        WHERE A.Tipo IN ('Asesor')
                          AND A.Estatus = 'Alta'
                          AND ISNULL(A.Jornada, '') != ''
    ...
    ELSE
        BEGIN
            DECLARE HorarioJornada CURSOR FOR
                ...
                FROM Agente
                WHERE Tipo IN ('Asesor', 'Mecanico')
                  AND Estatus = 'Alta'
                  AND NULLIF(Jornada, '') IS NOT NULL
...
------------------------------------------------------------------
    ...
    WHILE @@FETCH_STATUS = 0
        BEGIN
            ...
            SELECT @Hora = null
            SELECT @Inicio = MIN(CONVERT(varchar(5), Entrada, 108)),
                   @IFin = MIN(CONVERT(varchar(5), Salida, 108)),
                   @FinI = MAX(CONVERT(varchar(5), Entrada, 108)),
                   @Fin = MAX(CONVERT(varchar(5), Salida, 108))
            FROM JornadaTiempo
            WHERE Jornada in (@Jornada)
              AND YEAR(Fecha) = @Anio
              AND MONTH(Fecha) = @Mes
              AND DAY(Fecha) = @Dia
              AND Fecha NOT IN (SELECT Fecha FROM JornadaDiaFestivo)
            -------------------------------------------------------------------------------------------------------------------------
...
--- EXTRAE LOS ASESORES JUNTO CON SUS JORNADAS Y HORARIOS DISPONIBLES
            ...
            IF EXISTS(SELECT *
                ...
                BEGIN
                    DECLARE HorarioAgentes CURSOR FOR
                        SELECT A.Jornada, A.Agente, HJ.Hora
                        FROM Agente AS A
    ...
    ELSE
        BEGIN
            DECLARE HorarioAgentes CURSOR FOR
                ...
                FROM Agente AS A
                         INNER JOIN @HorariosJornadas AS HJ ON A.Jornada = HJ.Jornada
                WHERE A.Tipo IN ('Asesor', 'Mecanico')
                  AND A.Estatus = 'Alta'
                  --AND Familia = 'Recepcion'
                  AND NULLIF(A.Jornada, '') IS NOT NULL
                  AND A.SucursalEmpresa = @Sucursal;
    ...
    WHILE @@FETCH_STATUS = 0
        BEGIN
            ...
            SELECT @FechaIni = CONVERT(datetime,
                        @Anio + '-' + REPLACE(STR(@Mes, 2), ' ', '0') + '-' + REPLACE(STR(@Dia, 2), ' ', '0') + 'T' +
                        @Hora + ':00.000', 126)
                 , @FechaFin = DATEADD(MI, ISNULL(@Recepcion, 0), @FechaIni);
            ---SE BUSCA QUE NO HEXISTA UNA CITA U SERVICIO EN ESE HORARIO, DE EXISTIR SIMPLEMENTE NO CONTAMOS A ESE ASESOR PAR ESA HORA PROPUESTA Y SALTA AL SIGUIENTE----SE agrega cambio para aumentar un min a la fecha inicial de recorrido de hora para evitar descarte de 2 fechas consecutivas para un mismo agente
            IF NOT EXISTS(SELECT *
                          FROM #HorariosDisponibles
                          WHERE Agente = @Agente
                            AND (dtRequeridaIni BETWEEN @FechaIni AND @FechaFin OR
                                 DATEADD(MI, -1, dtRequeridaFin) BETWEEN @FechaIni AND @FechaFin)
                            AND Jornada = @Jornada)
                BEGIN
            ...
            IF EXISTS(SELECT *
                ...
                BEGIN
                    IF EXISTS(SELECT * FROM @HorarioAgentes)
                        BEGIN
                            SELECT H.Agente, A.Nombre, H.Fecha, H.Hora
                            FROM @HorarioAgentes AS H
                                     INNER JOIN Agente as A ON H.Agente = A.Agente
                                     INNER JOIN CA_HorarioAgenteRecepcion AS HO ON A.Agente = HO.Agente
                            WHERE Hora >= @HoraComienzo
                              AND HO.HoraFin >= Hora
                            ORDER BY Hora
                        END
                    ELSE
...
*/
CREATE PROCEDURE [dbo].[xpCA_HorarioAgente](@Sucursal int, @Anio VARCHAR(4), @Mes VARCHAR(2), @Dia VARCHAR(2),
                                            @Interfaz VARCHAR(20))
AS
BEGIN
    /*GTS|Inicio|Funcion para obtener un horario de dia dependiendo de la la hora de la jornada y los minutos de recepcion*/
    ...
    ELSE
        BEGIN
            ...
            --ELSE
            --BEGIN
            IF (SUBSTRING(CONVERT(VARCHAR(23), @FechaConMargen, 126), 12, 5)) > ('19:00') AND
               CONVERT(VARCHAR(10), @FechaConMargen, 103) =
               @Anio + '-' + REPLACE(STR(@Mes, 2), ' ', '0') + '-' + REPLACE(STR(@Dia, 2), ' ', '0')
                BEGIN
                    IF @Interfaz = 'SePaSlot'
                        BEGIN
    ...
    WHILE @@FETCH_STATUS = 0
        BEGIN
            ...
            ---SE agrega cambio para aumentar un min a la fecha inicial de recorrido de hora para evitar descarte de 2 fechas consecutivas para un mismo agente
            IF NOT EXISTS(SELECT *
                          FROM #HorariosDisponibles
                          WHERE Agente = @Agente
                            AND (dtRequeridaIni BETWEEN @FechaIni AND @FechaFin OR
                                 dtRequeridaFin BETWEEN DATEADD(minute, 1, @FechaIni) AND @FechaFin)
                            AND Jornada = @Jornada)
                BEGIN
                    INSERT INTO @HorarioAgentes Values (@Agente, @FechaIni, @Hora)
                END
        ...
        BEGIN
            IF EXISTS(SELECT *
                ...
                BEGIN
                    IF EXISTS(SELECT * FROM @HorarioAgentes)
                        BEGIN
                            SELECT H.Agente, H.Fecha, H.Hora
                            FROM @HorarioAgentes AS H
...
IF EXISTS(SELECT *
          FROM SYSOBJECTS
          WHERE ID = OBJECT_ID('dbo.xpCA_GeneraClienteSC')
            AND TYPE = 'P')
...
/* =============================================
 ...
 =============================================*/
CREATE PROCEDURE [dbo].[xpCA_GeneraClienteSC](@Nomre VARCHAR(50), @APaterno VARCHAR(50), @AMaterno VARCHAR(50),
                                              @CP VARCHAR(10)=NULL, @Email VARCHAR(100)=NULL, @Tel VARCHAR(15)=NULL)
AS
BEGIN
    SET NOCOUNT ON
    DECLARE
    ...
    IF (@Nomre IS NOT NULL AND @APaterno IS NOT NULL)
        BEGIN
            ...
            FROM CA_SKClientes
            WHERE PersonalNombres = @Nombres
              AND PersonalApellidoPaterno = @Apellido1
              AND PersonalApellidoMaterno LIKE @Apellido2 + '%'
            UNION
            SELECT Cliente, Nombre
            FROM CA_SKClientes
            WHERE Nombre = @Nombres + ' ' + @Apellido1 + ' ' + @Apellido2
            UNION
            SELECT Cliente, Nombre
            FROM CA_SKClientes
            WHERE Nombre LIKE @Nombres + ' ' + @Apellido1 + ' ' + @Apellido2 + '%'
            UNION
            SELECT Cliente, Nombre
            FROM CA_SKClientes
            WHERE Nombre LIKE @Nombres + ' ' + @Apellido1 + '% ' + @Apellido2 + '%'
            ...
            SELECT Cliente, Nombre
            FROM CA_SKClientes
            WHERE Nombre = @Apellido1 + ' ' + @Apellido2 + ' ' + @Nombres
            UNION
            SELECT Cliente, Nombre
            FROM CA_SKClientes
            WHERE Nombre LIKE @Apellido1 + '% ' + @Apellido2 + '% ' + @Nombres
    ...
    if ISNULL(@Contador, 0) > 0
        ...
        begin
            SELECT @Delegacion = SATMunicipioDescripcion,
                   @Colonia = SATColoniaDescripcion,
                   @Poblacion = SATLocalidadDescripcion,
                   @Estado = SATEstadoDescripcion,
                   @Pais = SATPaisDescripcion
            ...
            BEGIN TRY
                BEGIN TRANSACTION
                    INSERT INTO CA_SKClientes (Cliente, Estatus, Nombre, PersonalNombres, PersonalApellidoPaterno,
                                               PersonalApellidoMaterno, Delegacion, Colonia, Poblacion, CodigoPostal,
                                               Estado, Pais, Direccion, DireccionNumero, RFC, FiscalRegimen,
                                               TelefonosLada, Telefonos, PersonalTelefonoMovil, eMail1, Contactar,
                                               ContactarDe, ContactarA, Sexo)
                    VALUES (@Consecutivo, 'ALTA', UPPER(@Nomre + ' ' + @APaterno + ' ' + @AMaterno), UPPER(@Nomre),
                            UPPER(@APaterno), UPPER(@AMaterno), @Delegacion, @Colonia, @Poblacion, @CP, @Estado, @Pais,
                            'Conocido', '#', 'XAXX010101000', 'Persona Fisica', SUBSTRING(@Tel, 1, 3),
                            SUBSTRING(@Tel, 4, 10), SUBSTRING(@Tel, 1, 10), @Email, @Contacto, '09:00', '19:00',
                            'Masculino')
                COMMIT TRANSACTION
            END TRY
            BEGIN CATCH
...
IF EXISTS(SELECT *
          FROM SYSOBJECTS
          WHERE ID = OBJECT_ID('dbo.xpCA_BuquedaClinteSK')
            AND TYPE = 'P')
...
IF EXISTS(SELECT *
          FROM SYSOBJECTS
          WHERE ID = OBJECT_ID('dbo.xpCA_GenerarCitaSePa')
            AND TYPE = 'P')
    ...
    BEGIN TRY
        ...
        BEGIN TRANSACTION
            IF EXISTS (SELECT * FROM AgendaHora WHERE Hora >= SUBSTRING(CONVERT(VARCHAR(23), GETDATE(), 126), 12, 5))
                SELECT TOP 1 @HoraRecepcion = Hora
                FROM AgendaHora
                WHERE Hora >= SUBSTRING(CONVERT(VARCHAR(23), GETDATE(), 126), 12, 5)
            ELSE
                SELECT @HoraRecepcion = SUBSTRING(CONVERT(VARCHAR(23), GETDATE(), 126), 12, 5)

            IF EXISTS(SELECT * FROM SYSOBJECTS WHERE ID = OBJECT_ID('dbo.CA_sepa_conf_correo') and type = 'U')/*Revisamos si existe la tabla para poder sacar la Margenes de tiempo*/
                BEGIN
                    SELECT @Usuario = UsuarioSePaCS, @AgenteServicio = AgenteServicio
                    FROM CA_sepa_conf_correo
                    WHERE Sucursal = @Sucursal
                END
                ...
                BEGIN
                    ...
                    IF @Agente IN ('AgenteDef', '')
                        BEGIN
                            ...
                            SELECT top 1 @Agente = Agente.Agente
                            FROM Agente
                                     INNER JOIN JornadaTiempo AS JT ON JT.Jornada = Agente.Jornada
                            WHERE Agente.Jornada is not null
                              and Tipo = 'Asesor'
                              AND Estatus = 'ALTA'
                              and sucursalempresa = @Sucursal
                    ...
                    SELECT TOP 1 @SArt = ServicioArticulo,
                                 @SModelo = ServicioModelo,
                                 @SPlacas = ServicioPlacas,
                                 @SVin = ServicioSerie,
                                 @ArticuloPaquete = ISNULL(ArticuloPaquete, ''),
                                 @Concepto = Concepto
                    FROM CA_log_sepa_citas
                    ...
                    IF @Concepto != (SELECT TOP 1 Concepto
                                     FROM Venta
                                     where Mov = 'cita Servicio' AND Concepto LIKE 'Publico%'
                                     order by ID desc)
                        BEGIN
                            SELECT TOP 1 @Concepto = Concepto
                            FROM Venta
                            where Mov = 'cita Servicio' AND Concepto LIKE 'Publico%'
                            order by ID desc
                        END
                    ...
                    INSERT INTO Venta
                    ( Empresa, Mov, FechaEmision, Concepto, UEN, Moneda, TipoCambio, Usuario, Estatus, Cliente, Almacen
                    , Agente, FechaRequerida, HoraRequerida, HoraRecepcion
                    , Condicion, ServicioArticulo, ServicioSerie, ServicioPlacas, ServicioKms, Ejercicio, Periodo
                    , ListaPreciosEsp, Sucursal, Comentarios, SucursalOrigen, ServicioTipoOrden, ServicioTipoOperacion
                    , ServicioModelo, ServicioNumeroEconomico, ServicioDescripcion, AgenteServicio)
                    ...
                    SELECT Empresa,
                           Mov,
                           CONVERT(VARCHAR(10), FechaEmision, 126),
                           @Concepto,
                           dbo.fnCA_GeneraUENValida('VTAS', Mov, dbo.fnCA_GeneraSucursalValida('VTAS', Mov, Sucursal),
                                                    @Concepto),
                           Moneda,
                           TipoCambio,
                           ...
                           Estatus,
                           @ClienteSK,
                           dbo.fnCA_GeneraAlmacenlValido('VTAS', Mov,
                           ...
                           ListaPreciosEsp,
                           dbo.fnCA_GeneraSucursalValida('VTAS', Mov, Sucursal),
                           'Creada desde Interfaz Seekop ' + CHAR(10) + Comentarios,
                           SucursalOrigen,
                           ServicioTipoOrden,
                    ...
                    IF EXISTS (SELECT *
                               ...
                               WHERE TABLE_NAME = 'CA_Venta' AND TABLE_SCHEMA = 'dbo')/*Revisa si trae la nomenclatura CA_ en la tabla para buscar tablas de la V6000 */
                        BEGIN
                            INSERT INTO CA_Venta(ID, SKCte, SKNombreCompleto, SKNombres, SKAPaterno, SKAMaterno,
                                                 SKArtVehiculo, SKModelo, SKPlacas, SKVin)
            ...
            ELSE
                BEGIN
                    ...
                    SELECT Empresa,
                           ...
                           ListaPreciosEsp,
                           dbo.fnCA_GeneraSucursalValida('VTAS', Mov, Sucursal),
                           'Creada desde APP SePa ' + CHAR(10) + Comentarios,
                           SucursalOrigen,
                           ServicioTipoOrden,
                           ServicioTipoOperacion,
                           ServicioModelo,
            ...
            INSERT INTO CA_IDSPendientesConfirmarCita (IDVenta, Estatus, FechaProcesamiento)
            VALUES (@GenerarID, 'SINPROCESAR', GETDATE())
            ...
            IF EXISTS(SELECT * FROM Venta WHERE ID = @GenerarID)--AND Estatus IN ('CONFIRMAR'))
                BEGIN
                    SELECT /*MovID*/ CONVERT(VARCHAR(10), @GenerarID) AS Folio, '' AS OkRef
                    FROM Venta
                    WHERE ID = @GenerarID
                    ...
                    IF @Cliente LIKE 'SKS%'
                        BEGIN
                            ...
                            SELECT V.Sucursal,
                                   ...
                                   NULL,
                                   'SeekopCitas',
                                   (SELECT TOP 1 VERSION FROM Sucursal)
                            FROM Venta AS V
                            WHERE V.ID = @GenerarID
                            SELECT @IDLog = IDENT_CURRENT('CA_LogMovimientosInterfaz')
                            /*Detalle*/
                            INSERT INTO CA_LogMovimientosInterfazD
                            SELECT @IDLog,
                                   CONVERT(VARCHAR(6), @OK),
                                   CASE WHEN @OK IS NULL THEN '1' ELSE '1' END,
                                   NULL,
                                   @OkRef
                            /*INICIO:Insertamos Informacion en el Monitor General de Interfaces*/
                            ...
                            IF @ArticuloPaquete != ''
                                BEGIN
                                    INSERT INTO VENTAD (ID, Renglon, RenglonSub, RenglonID, RenglonTipo, Almacen, UEN,
                                                        Sucursal, SucursalOrigen, Cantidad, Articulo, Impuesto1,
                                                        DescripcionExtra, UT, CCTiempoTab)
                                    SELECT @GenerarID,
                                           2048,
                                           0,
                                           1,
                                           ...
                                           ISNULL(Horas, 1)
                                    FROM ART
                                    WHERE ARTICULO = @ArticuloPaquete
                                END
                        END
                END
...
IF EXISTS(SELECT *
          FROM SYSOBJECTS
          WHERE ID = OBJECT_ID('dbo.xpCA_CancelarCitaSePa')
            AND TYPE = 'P')
...
BEGIN
    ...
    IF EXISTS (SELECT * FROM CA_log_sepa_citas WHERE Id_sepa_Citas = @ID AND Estatus = 'CONFIRMAR')
        BEGIN
            SELECT @Fecha = FechaEmision, @Hora = HoraRequerida, @IDIntelisis = Id_CitaIntelisis
            FROM CA_log_sepa_citas
            WHERE Id_sepa_Citas = @ID
    ...
    ELSE
        BEGIN
            IF EXISTS (SELECT * FROM CA_log_sepa_citas WHERE Id_sepa_Citas = @ID AND Estatus = 'CANCELADO')
                BEGIN
                    SELECT 'CANCELADO'                                AS 'Estatus',
                           'Este folio ya fue Cancelado Previamente.' AS 'Respuesta',
                           '404'                                      AS 'Tipo'
                END
            ELSE
    ...
    BEGIN TRANSACTION
        ...
        /*Afectamos la CS*/
        EXEC spAfectar 'VTAS', @IDIntelisis, 'CANCELAR', 'Todo', NULL, @Usuario, @Estacion=1000, @EnSilencio=1,
...
IF EXISTS(SELECT *
          FROM SYSOBJECTS
          WHERE ID = OBJECT_ID('dbo.xpCA_ActualizacionEstatusMonitor')
            AND TYPE = 'P')
...
BEGIN
    ...
    SELECT @Mov = ISNULL(V.Mov, ''), @Estatus = ISNULL(V.Estatus, '')
    FROM Venta AS V
             INNER JOIN CA_Venta AS CV ON V.ID = CV.ID
    WHERE SKCte LIKE 'SKS%'
      AND V.ID = @ID
    ...
    IF @Mov = 'Cita Servicio' AND @Estatus IN ('CONCLUIDO', 'CANCELADO')
        BEGIN
            IF EXISTS(SELECT *
                      ...
                      WHERE IDDocumento = @ID AND Mov = @Mov AND Interfaz = 'SeekopCitas')
                BEGIN
...
IF EXISTS(SELECT *
          FROM SYSOBJECTS
          WHERE ID = OBJECT_ID('dbo.xpCA_ConcluirCitaSePa')
            AND TYPE = 'P')
...
BEGIN
    ...
    IF EXISTS (SELECT *
               FROM INFORMATION_SCHEMA.TABLES
               WHERE TABLE_NAME = 'CA_InterfazRutaArchivo' AND TABLE_SCHEMA = 'dbo')/*Revisa si trae la nomenclatura CA_ en la tabla para buscar tablas de la V6000 */
        BEGIN
    ...
    IF EXISTS (SELECT * FROM CA_log_sepa_citas WHERE Id_CitaIntelisis = @ID AND Cliente LIKE 'SKS%')
        BEGIN
            ...
            SELECT @Url =
                   dbo.fnCA_BusquedaClaveParametroInterfazEmpresa('SeekopCitas', 'SKEndPointConcludeAppointments')
    ...
    IF EXISTS (SELECT * FROM CA_log_sepa_citas WHERE Id_CitaIntelisis = @ID AND Estatus = 'CONFIRMAR')
        BEGIN
            SELECT @Json =
                   '"{' + char(34) + char(34) + 'cliente' + char(34) + char(34) + ': ' + char(34) + char(34) + Cliente +
                   char(34) + char(34) + ','
                       + char(34) + char(34) + 'folio' + char(34) + char(34) + ': ' + char(34) + char(34) +
                   CONVERT(VARCHAR(50), Id_sepa_Citas) + char(34) + char(34) + ','
                       + char(34) + char(34) + 'empresa' + char(34) + char(34) + ': ' + char(34) + char(34) + Empresa +
                   char(34) + char(34) + '}"',
                   @Accion = '2'
            FROM CA_log_sepa_citas
            WHERE Id_CitaIntelisis = @ID
...
IF EXISTS(SELECT *
          FROM SYSOBJECTS
          WHERE ID = OBJECT_ID('dbo.xpCA_SlotsHorarios')
            AND TYPE = 'P')
                            ...
                            IF @Hora < @Fin
                                BEGIN
                                    ...
                                    SELECT @Fecha,
                                           0,
                                           @Hora,
                                           CONVERT(varchar(5), DATEADD(MI, @Recepcion, ISNULL(@Hora, @Inicio)), 108),
                                           0,
                                           @Sucursal
                                END
...
----------------------------------------------------------
    ...
    UPDATE #CA_SlotHorarios
    SET DiaHabil=1
    WHERE Fecha IN (SELECT DISTINCT Fecha FROM #CA_SlotHorarios WHERE Disponible = 1)
...
IF EXISTS(SELECT *
          FROM SYSOBJECTS
          WHERE ID = OBJECT_ID('dbo.xpCA_DespuesAfectarAgendamientoCitasSeekop')
            AND TYPE = 'P')
